...
export const getAdminProducts = ()=>async(dispatch)=>{
    try {
        ...
        let {data} = await axios.post(`${baseUrl}/api/v1/admin/products`,{token},config);
...
export const getAllProductsForHome = ()=>async(dispatch)=>{
    try {
        dispatch({type:ADMIN_PRODUCT_REQUEST});
        
        let {data} = await axios.get(`${baseUrl}/api/v1/getallproducts`);
...
export const newReiview = (Data)=> async(dispatch)=>{
    try {
        ...
        let token = localStorage.getItem("Token");
        let {data} = await axios.put(`${baseUrl}/api/v1/review`,{reviewData,token},config);
...
export const deleteProduct = (id) => async(dispatch)=>{
    try {
        ...
        let {data} = await axios.delete(`${baseUrl}/api/v1/admin/product/${id}`,{data:{token}});
