...
export const MyOrdersp = () => async(dispatch,getState) =>{
    try {
        ...
        const {data} = await axios.post(`${baseUrl}/api/v1/orders/me`,{token},config);
...
export const OrderDetailsp = (id)=> async(dispatch)=>{
    try {
        ...
        const {data} = await axios.post(`${baseUrl}/api/v1/order/${id}`,{token},config);
        console.log(data);
...
export const getAllOrders = () => async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(`${baseUrl}/api/v1/admin/orders`,{token},config);
  ...
  export const deleteOrder = (id) => async (dispatch) => {
    try {
      ...
      let token = localStorage.getItem("Token");
      const { data } = await axios.delete(`${baseUrl}/api/v1/admin/order/${id}`,{data:{token}});
