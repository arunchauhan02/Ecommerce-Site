...
export const login = (email,password) => async(dispatch)=>{
    try {
        ...
        let {data} = await axios.post("/api/v1/login",{email,password},config);
        console.log(data);
...
export const logout = ()=>async(dispatch)=>{
    try {
        let {data} = axios.get("/api/v1/logout");
...
export const registerUser = (userData)=> async (dispatch)=>{
    try {
        ...
        let {data} = await axios.post("/api/v1/register",userData,config);
...
export const updatePassword = (userData)=> async (dispatch)=>{
    try {
        ...
        const config = {headers:{'Content-Type':"application/json"}};
        let {data} = await axios.put("/api/v1/password/update",userData,config);
...
export const forgotPassword = (email)=> async(dispatch)=>{
    try {
        ...
        const config = {headers:{'Content-Type':"application/json"}};
        let {data} = await axios.post("/api/v1/password/forget",email,config);
...
export const resetPassword = (token,passwords)=>async(dispatch)=>{
    try {
        ...
        let {data} = await axios.put(`/api/v1/password/reset/${token}`,passwords,config);
